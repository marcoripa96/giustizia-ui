...
const ClusterContainer = styled.button<{ selected: boolean }>(({ selected }) => ({
  position: 'relative',
  display: 'flex',
...
const ClusterCard = ({ id, mentions, type, title, selected, onClick }: ClusterCardProps) => {
  ...
  return (
    <>
      <ClusterContainer selected={selected} onClick={onClick}>
        ...
        <Text b>{title}</Text>
        <Text size="12px">{t('leftSidebar.clustersContent.mentions', { n: mentions.length })}</Text>
